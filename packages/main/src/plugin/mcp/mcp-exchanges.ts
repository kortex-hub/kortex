...
export class MCPExchanges {
  ...
  public createMiddleware(instance: MCPInstance): Transport {
    // Wrap transport with delegate to record all exchanges
    return new MCPTransportDelegate(instance.transport, {
      onSend: (message): void => {
        if (isJSONRPCRequest(message)) {
          this.recordInput(instance.configId, message);
        }
      },
      onReceive: (message, _extra): void => {
        if (isJSONRPCResponse(message)) {
          this.recordOutput(instance.configId, message);
        }
      },
    });
  }
