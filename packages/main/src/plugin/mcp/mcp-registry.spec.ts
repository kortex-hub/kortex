...
describe('MCPRegistry - listMCPServersFromRegistry', () => {
  ...
  test('should handle invalid server data gracefully with warnings', async () => {
    const responseWithInvalidServer = {
      servers: [
        {
          server: {
            name: 'test-server',
            ...
            version: '1.0.0',
            // No $schema declared, so no per-server validation
          },
          _meta: {},
        },
      ],
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => responseWithInvalidServer,
    });
  ...
  test('should handle response even with missing _meta field', async () => {
    const responseWithMissingMeta = {
      servers: [
        {
          server: {
            name: 'test-server',
          ...
          // _meta field missing but we're lenient on ServerList structure
        },
      ],
    };
    ...
    fetchMock.mockResolvedValueOnce({
      ok: true,
      json: async () => responseWithMissingMeta,
    });

    // Should not throw - we're lenient on ServerList structure
    const result = await mcpRegistry.testListMCPServersFromRegistry('https://registry.example.com');
    expect(result.servers).toHaveLength(1);
    expect(result.servers[0]?.server.name).toBe('test-server');
  });

  test('should handle servers with invalid data gracefully', async () => {
      ...
      servers: [
        {
          server: {
            ...
            description: 'Test',
            version: '1.0.0',
            // No $schema, so no per-server validation happens
          },
          _meta: {},
        },
      ],
