...
export class SchedulerRegistry implements IDisposable {
  ...
  async init(): Promise<void> {
    ...
    this.ipcHandle(
      ...
      async (_listener, metadataKeys: string[]): Promise<ProviderScheduleItemInfo[]> => {
        return this.listItems(metadataKeys);
      },
    );
  ...
  async schedule(schedulerName: string, options: ProviderSchedulerOptions): Promise<ProviderScheduleResult> {
    const scheduler = this.#schedulers.get(schedulerName);
    if (!scheduler) {
      return Promise.reject(new Error(`Scheduler with name ${schedulerName} not found`));
    }
