...
export function getDatabaseName(providerInfos: ProviderInfo[], ragEnvironment: RagEnvironment | undefined): string {
  ...
  const ragConnection = ragProvider?.ragConnections.find(connection => connection.name === ragEnvironment?.ragConnection.name);
  return ragConnection?.name ? `${ragConnection.name} (${ragProvider?.name})` : `N/A`;
}

export function getChunkProviderName(chunkProviders: ChunkProviderInfo[], ragEnvironment: RagEnvironment | undefined): string {
