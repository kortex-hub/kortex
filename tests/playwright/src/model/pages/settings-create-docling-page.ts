...
export class SettingsCreateDoclingPage extends BasePage {
  ...
  constructor(page: Page) {
    super(page);
    this.nameInput = page.getByLabel('Name for the Docling chunk provider connection');
  ...
  async goBackToResources(): Promise<void> {
    await expect(this.goBackToResourcesButton).toBeVisible({ timeout: TIMEOUTS.IMAGE_PULL });
