...
export const TOKENS_KEY = 'anthropic:tokens';
export const TOKEN_SEPARATOR = ',';

export class Anthropic implements Disposable {
  ...
  private async getTokens(): Promise<string[]> {
    ...
    try {
      ...
      console.error('Anthropic: something went wrong while trying to get tokens from secret storage', err);
    }
  ...
  private async registerInferenceProviderConnection({ token }: { token: string }): Promise<void> {
    if (!this.provider) throw new Error('cannot create inference provider connection: provider is not initialized');
  ...
  private async mcpFactory(params: { [p: string]: unknown }): Promise<void> {
    ...
    const apiKey = params['anthropic.factory.apiKey'];
